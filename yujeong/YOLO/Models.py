...
def create_modules(module_defs):
    ...
    return hyperparams, module_list     #맨 처음 모듈, 그 외 나머지 모듈들
...
class Darknet(nn.Module):
    ...
    def __init__(self, config_path, img_size=416):
        ...
        self.module_defs = parse_model_config(config_path)         #yolov3-tiny-onecls 파일 저장
        self.hyperparams, self.module_list = create_modules(self.module_defs)   #hyperparams = net 모듈, modele_list = 그 외 나머지 모듈들
        self.yolo_layers = [layer[0] for layer in self.module_list if hasattr(layer[0], "metrics")]
        self.img_size = img_size
        self.seen = 0
        self.header_info = np.array([0, 0, 0, self.seen, 0], dtype=np.int32)

    def forward(self, x, targets=None): # yolov3-tiny-onecls 토큰화
        img_dim = x.shape[2]
        loss = 0
